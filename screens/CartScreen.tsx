...
const CartScreen: React.FC = () => {
  const { placeOrder, formatPrice, products, paymentAliases } = useApp();
  ...
  if (isProcessing) {
      return (
          <div className="bg-[#f8f6f5] dark:bg-[#221510] min-h-screen flex flex-col items-center justify-center font-display">
              ...
              <p className="text-gray-500 text-sm">Conectando con Mercado Pago Split</p>
              <p className="text-[10px] text-gray-600 mt-2">Distribuyendo fondos a {paymentAliases.merchant ? 'Comercio' : '...'} y {paymentAliases.rider ? 'Rider' : '...'}</p>
          </div>
      )
  }
      ...
      <main className="max-w-lg mx-auto pb-48">
        ...
        <div className="px-4 mt-6">
            <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase text-gray-500">Pago Inteligente (Split)</span>
            </div>
            <div className="bg-[#009ee3]/10 border border-[#009ee3]/30 p-3 rounded-lg flex gap-3 items-start">
                <span className="material-symbols-outlined text-[#009ee3]">call_split</span>
                <p className="text-xs text-[#009ee3] font-medium leading-relaxed">
                    Tu pago se divide automáticamente en tiempo real:
                    <br/>• <strong>{formatPrice(subtotal)}</strong> &rarr; Alias: {paymentAliases.merchant || 'Comercio'}
                    <br/>• <strong>{formatPrice(2000)}</strong> &rarr; Alias: {paymentAliases.rider || 'Rider'}
                    <br/>• <strong>{formatPrice(1500)}</strong> &rarr; Manda2
                </p>
            </div>
        </div>
      </main>
