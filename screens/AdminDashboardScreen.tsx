  ...
  return (
    <div className="bg-[#f6f7f8] dark:bg-[#101822] text-slate-900 dark:text-white min-h-screen font-inter">
      ...
      <main className="max-w-md mx-auto pb-24">
        ...
        {/* SECCIÓN DE LIQUIDACIONES (NUEVO) */}
            ...
            <div className="bg-white dark:bg-[#1a232e] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-white/5 rounded-lg">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">A pagar a Riders</p>
                            <p className="text-xs text-gray-400">Alias: riders.manda2</p>
                        </div>
                        <p className="text-lg font-bold text-red-500">-{formatPrice(totalRiderDebt)}</p>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-white/5 rounded-lg">
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">A pagar a Comercios</p>
                            <p className="text-xs text-gray-400">Alias: comercios.manda2</p>
                        </div>
                        <p className="text-lg font-bold text-red-500">-{formatPrice(totalMerchantDebt)}</p>
                    </div>
                    <div className="border-t border-gray-200 dark:border-gray-700 my-2 pt-2 flex justify-between items-center">
                        <p className="text-sm font-bold text-slate-700 dark:text-white">Ganancia Neta Plataforma</p>
                        <p className="text-xl font-bold text-green-500">+{formatPrice(totalPlatformRevenue)}</p>
                    </div>
                    <button className="w-full bg-[#136dec] hover:bg-blue-700 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-colors">
                        <span className="material-symbols-outlined">send_money</span>
                        Marcar como Liquidado
                    </button>
                    <p className="text-[10px] text-center text-gray-400">
                        * Recuerda hacer las transferencias manualmente desde tu banco a los Alias correspondientes.
                    </p>
                </div>
            </div>
        </section>
